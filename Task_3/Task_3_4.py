...
if D.imag == 0:
    if D.real > 0:
        ...
        if z2.imag == 0 or z1.imag == 0:
            ...
            print("Корни {:.2f}{:+.2f}j, {:.2f}{:+.2f}j".format(z1.real, z1.imag, z2.real, z2.imag))
    if D.real == 0:
        z = complex(-b.real / (2*a), -b.imag / (2*a))
        if z.imag == 0:
            print("Корень {:.2f}".format(z.real))
        else:
            print("Корень {:.2f}{:+.2f}j".format(z.real, z.imag))
    if D.real < 0:
        z1 = complex((-b.real) / (2*a), (-b.imag+m.sqrt(-D.real)) / (2*a))
        z2 = z1.conjugate()
        print("Корни {:.2f}{:+.2f}j, {:.2f}{:+.2f}j".format(z1.real, z1.imag, z2.real, z2.imag))
else:
    print(round(D.imag))
